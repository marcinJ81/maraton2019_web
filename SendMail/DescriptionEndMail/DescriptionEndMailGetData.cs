...
namespace SendMail.DescriptionEndMail
{
    ...
    {
        public Abstract_And_Model_Layer.Mail_message_Model.DescriptionEndMail get_DataFoDescritpion(string imie, string nazwisko, string email)
        {
            try
            {
                using (var db = new EntitiesRegistrationParticipant())
                {
                    ...
                    var result_TMP = result_DIK.Select(x => new Abstract_And_Model_Layer.Mail_message_Model.DescriptionEndMail
                    {
